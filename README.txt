...
If the data is correct it will return a JSON encoded object representing the data sent from the LoRa Strip.

API KEYS (OPTIONAL):

Additionally, should you wish you can add rudimentary API key handling to manage who can make calls to this API. 
The API keys are intended to be secret but not severely secret since this only prevents decoding (the HTTPS layer is 
responsible for data security). In this case, add a folder apikeys in the installation directory. 
Should it be there and be non-empty, all of the file names in there will serve as API keys (which can be added or 
removed if a particular key holder is added or barred).

In this case, a new parameter k is read, and matched against the file names in the apikeys folder.

Example: host:{PORT}/translate?p={LORA PORT}&d={HEXADECIMAL LORA PAYLOAD}&k={Some random file name, for instance a base64 encoded hash}
